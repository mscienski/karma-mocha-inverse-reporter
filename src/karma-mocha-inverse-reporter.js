 ...
 * @constructor
 */
var MWSReporter = function (baseReporterDecorator, formatError, config) {
    ...
    config.mwsReporter = config.mwsReporter || {};

    var outputMode = config.mwsReporter.output || 'full';
    var ignoreSkipped = config.mwsReporter.ignoreSkipped || false;
    var divider = config.mwsReporter.divider || '=';
    ...
    // set color functions
    config.mwsReporter.colors = config.mwsReporter.colors || {};

    // set diff output
    config.mwsReporter.showDiff = config.mwsReporter.showDiff || false;

    var colors = {
        success: {
            symbol: symbols.success,
            print: chalk[config.mwsReporter.colors.success] || chalk.green
        },
        info: {
            symbol: symbols.info,
            print: chalk[config.mwsReporter.colors.info] || chalk.grey
        },
        warning: {
            symbol: symbols.warning,
            print: chalk[config.mwsReporter.colors.warning] || chalk.yellow
        },
        error: {
            symbol: symbols.error,
            print: chalk[config.mwsReporter.colors.error] || chalk.red
        }
    };

    // check if mocha is installed when showDiff is enabled
    if (config.mwsReporter.showDiff) {
        try {
    ...
    function printFailures (suite, depth) {
        ...
        for (i = length - 1; i >= 0; i--) {
            ...
            if (item.name && !item.success && !item.skipped) {
                // indent
                var line = repeatString('  ', depth) + ((item.type === 'it') ? (i + ') ' + item.name) : item.name);
                ...
                if (item.type === 'it') {
                    ...
                    // print diff
                    if (config.mwsReporter.showDiff && item.assertionErrors && item.assertionErrors[0]) {
                        ...
                        log.forEach(function (err) {
                            line += repeatString('  ', depth) + colors.error.print(formatError(err));
                        });

                        // create diff
                        var diff = config.mwsReporter.showDiff === 'inline' ? inlineDiff(err) : unifiedDiff(err);
...
MWSReporter.$inject = ['baseReporterDecorator', 'formatError', 'config'];

// PUBLISH DI MODULE
module.exports = {
    'reporter:mws': ['type', MWSReporter]
};
